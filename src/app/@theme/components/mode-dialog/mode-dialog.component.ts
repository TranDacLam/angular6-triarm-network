...
export class ModeDialogComponent implements OnInit, OnChanges {

    @Output() on_untrust_asset_from_mode_dialog = new EventEmitter();
    @Output() on_confirm_mode_dialog = new EventEmitter();
    ...
    goto(where){
        return this.router.navigate(['/wallet/transaction', {mode: where, wallet: this.walletModeDialog._id, asset: this.asset, amount: this.balance}]);
    }
    ...
    confirmTrust(){
        this.on_confirm_mode_dialog.emit();
        $('#modeDialog').modal('hide');
    }
}

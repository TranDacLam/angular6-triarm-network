...
@Component({
	selector: 'app-new-asset-multisign-user',
    templateUrl: './new-asset.html',
    styleUrls: ['./new-asset.scss']
})

export class NewAssetMultisignUserIssueComponent implements OnInit {
    ...
    async onConfirmTfa(event){
        if(this.setTypeConfirmTfa === 'onSetWalletProperties'){
            ...
            let response = await this.multiSignUserService.setWalletPropertyMultiSignForUser(data);
        ...
        }else if(this.setTypeConfirmTfa === 'createAsset'){
    ...
    async callAPIDisableWallet(asset, tfaCode){
        ...
        let custom_signers = [this.signers.signer1, this.signers.signer2, this.signers.signer3].filter(item => item);
        let data = {
            code: tfaCode,
            walletID: asset.walletID._id,
            signers: custom_signers
        };
        let response = await this.multiSignUserService.disablePaymentOfWalletMultiSignForUser(data);
    ...
    createAsset() {
        this.setTypeConfirmTfa = 'createAsset';
        $('#tfaConfirmDialog').modal('show');
    }
